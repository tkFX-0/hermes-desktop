...
setChihayaMemoryDir(MEMORY_DIR);
const REQUIRED_DIRS = [
  join(BASE, "docs", "logs"),
...
import {
  chihayaHandleCommand,
  checkKillZoneAlert,
  setChihayaHold,
  setChihayaMemoryDir,
  clearKillZoneAlertState,
} from "./shikishima-chihaya.mjs";
import {
  setAllFxNotifications,
  formatFxNotificationStatus,
  isChihayaHeld,
  shouldSendMarketReports,
...
async function sendViaWebhook(webhookUrl, agentId, content, { skipDedupe = false } = {}) {
...
const AGENT_PROMPTS_DEFAULT = {
  shikishima: "あなたは「しきしま(🏯)」です。秘書兼管制塔。ユーザーへの親しいため口で、推論は丁寧に。FXはちはや担当。",
...
async function handleMessage(content, opts = {}) {
  ...
  if (
    isChihayaHeld(MEMORY_DIR) &&
    /mt5|口座|fx|xauusd|gold|ea|kill.?zone|silver.?bullet|pips|lot|ドローダウン|dd|チャレンジ|atfunded/i.test(
      trimmed,
    )
  ) {
    return {
      agentId: "chihaya",
      replyText:
        "📈 **ちはや** — HOLD中のためFX応答を停止しています。\n再開: `!chihaya-start` / `!fx-on` / 「ちはや再開」",
    };
  }
  ...
  const contextBudget = opts.channelId ? 2200 : 900;
  const contextParts = [threadCtx, memCtx, taskCtx, goalCtx, mt5Ctx, monCtx, prefCtx]
...
async function sendReply(channelId, token, agentId, text, { bypassDedupe = false } = {}) {
  ...
  let body = null;
  if (_webhookUrl) {
    const r = await sendViaWebhook(_webhookUrl, agentId, safe, { skipDedupe: bypassDedupe });
...
/** Discord返答全文を VOICEVOX 経由で発話（グローバル直列キュー経由・同時発話なし）。 */
async function maybeEnqueueDiscordVoice({ userContent, replyText, agentId, source = "discord_reply" }) {
  ...
  if (!st.voicevoxReady || !st.connected) {
    console.warn("[StackChanVoice] skip — VOICEVOX or StackChan offline");
    return { ...decision, speak: false, reason: "device_not_ready" };
  }
  for (let i = 0; i < decision.chunks.length; i++) {
    const reason = `${decision.reason}:${i + 1}/${decision.chunks.length}`;
    void stackchanSay(decision.chunks[i], {
      skipMotion: true,
      skipMilestone: true,
      alreadyPrepared: true,
      queueLabel: `discord:${reason}`,
    }).then((r) => {
      const tag = r?.ok ? "ok" : (r?.error ?? r?.blockedReason ?? r?.skipped ?? "fail");
      console.log(`[StackChanVoice] chunk ${tag} (${reason}) pending=${getGlobalSpeechPendingCount()}`);
    });
  }
  auditLog({
  ...
  console.log(
    `[StackChanVoice] queued ${decision.chunks.length} chunk(s) (${decision.reason})`
      + ` globalPending=${getGlobalSpeechPendingCount()}`
...
const EXCLUSIVE_SLASH_CMD =
  /^!(dev-pipeline|human-go|governance|reply-status|obsidian-status|chihaya-stop|chihaya-start|chihaya-status|fx-off|fx-on)\b/i;
...
async function handleExclusiveSlashCommands(content, channelId, token) {
  ...
  if (/^!(chihaya-stop|fx-off)\b/i.test(t)) {
    setAllFxNotifications(MEMORY_DIR, false);
    clearKillZoneAlertState();
    const out = await sendOpsReply(
      channelId,
      token,
      "chihaya",
      `${formatFxNotificationStatus(MEMORY_DIR)}\n\n✅ FX関連の定期通知を停止しました。`,
    );
    if (out?.id) lastMessageId = out.id;
    return true;
  }

  if (/^!(chihaya-start|fx-on)\b/i.test(t)) {
    setAllFxNotifications(MEMORY_DIR, true);
    const out = await sendOpsReply(
      channelId,
      token,
      "chihaya",
  ...
  if (/^!chihaya-status\b/i.test(t)) {
    const out = await sendOpsReply(channelId, token, "chihaya", formatFxNotificationStatus(MEMORY_DIR));
    if (out?.id) lastMessageId = out.id;
    return true;
  }
...
async function poll(channelId, token) {
  ...
  _pollInFlight = true;
  const savedGlobalCursor = lastMessageId;
  ...
  try {
    ...
    for (const msg of newMsgs) {
      ...
      if (isUserOpsSlashCommand(content)) {
        const handled = await handleExclusiveSlashCommands(content, channelId, token);
        if (!handled) {
          await sendReply(
            channelId,
            token,
      ...
      // ちはや: HOLD/再開（!fx-* / !chihaya-* は専用ハンドラで処理済み）
      if (
        /ちはや.*stop|ちはや.*停止|ちはや.*hold|fx.*(停止|止め)|@ちはや\s+stop/i.test(content)
      ) {
        setAllFxNotifications(MEMORY_DIR, false);
        clearKillZoneAlertState();
      ...
      if (/ちはや.*再開|ちはや.*start|ちはや.*resume|fx.*(再開|開始)/i.test(content)) {
        setAllFxNotifications(MEMORY_DIR, true);
        const out = await sendReply(
          channelId,
          token,
      ...
      const voiceDecision = await maybeEnqueueDiscordVoice({
        userContent: content,
        replyText,
        agentId,
        source: "discord_reply",
      }).catch((e) => {
        console.warn("[StackChanVoice]", e.message);
        return { speak: false, reason: "enqueue_error" };
      });
    ...
    lastMessageId = savedGlobalCursor;
    _pollInFlight = false;
  }
}
...
function startAutonomousOrchestratorLoop() {
  const release = readOperationalRelease(BASE);
  if (!release.autonomousOrchestratorEnabled) {
    console.log("[Orchestrator] HOLD — phase GO: shikishima-phase-go.mjs ack autonomous_orchestrator");
    return;
  }
