...
const DEFAULT_DRAFT_OUTBOX = [
  {
    id: "phase-75-90-evidence-email-draft",
    title: "Evidence review email draft",
    summary: "Draft-only message for a human to review Phase 75 to 90 evidence.",
    proposedBy: "system",
    actionKind: "email_draft",
    riskLevel: "medium",
    draftState: "waiting_human",
    destinationLabel: "human-selected recipient",
    bodyPreview: "Phase 75 to 90 draft outbox evidence is ready for review. No email has been sent.",
    requiredHumanAction: "Review and manually send only if appropriate.",
    blockedReason: "This UI cannot send email.",
    safeNextStep: "Copy the draft text manually after human review.",
    evidenceRef: "PHASE_75_TO_90_DRAFT_OUTBOX_SAFETY_EVIDENCE.md",
    externalWrite: false,
    sent: false,
    remoteCreated: false,
  ...
  {
    id: "github-follow-up-draft",
    title: "GitHub follow-up draft",
    summary: "Draft-only issue or PR follow-up text. No GitHub remote object is created.",
    proposedBy: "codex",
    actionKind: "github_issue_draft",
    riskLevel: "medium",
    draftState: "waiting_human",
    destinationLabel: "GitHub manual review",
    bodyPreview: "Draft issue text only. Human may copy it into GitHub after separate approval.",
    requiredHumanAction: "Review draft and create remotely by hand only after GO.",
    blockedReason: "Remote issue or PR creation is forbidden in this phase.",
    safeNextStep: "Keep as draft until human approves external action.",
    evidenceRef: "ROADMAP_CHANGELOG.md",
    externalWrite: false,
    sent: false,
    remoteCreated: false,
  ...
  {
    id: "calendar-checkpoint-draft",
    title: "Calendar checkpoint draft",
    summary: "Draft-only calendar note for future human review. No calendar event is created.",
    proposedBy: "system",
    actionKind: "calendar_draft",
    riskLevel: "medium",
    draftState: "held",
    destinationLabel: "manual calendar entry",
    bodyPreview: "Suggested review checkpoint. Human may create the event manually later.",
    requiredHumanAction: "Separate calendar GO required.",
    blockedReason: "Calendar writes are external actions and remain HOLD.",
    safeNextStep: "Leave as display-only draft.",
    evidenceRef: "DEVELOPMENT_TEMPO_DASHBOARD.md",
    externalWrite: false,
    sent: false,
    remoteCreated: false,
    paymentOrReservation: false,
  ...
  {
    id: "purchase-reservation-hold",
    title: "Purchase or reservation action",
    summary: "Purchases, reservations, and payment-related actions remain human-only.",
    proposedBy: "system",
    actionKind: "purchase_or_reservation_draft",
    riskLevel: "critical",
    draftState: "held",
    destinationLabel: "human-only",
    bodyPreview: "No payment, purchase, reservation, or account operation has been prepared for execution.",
    requiredHumanAction: "Human-only decision outside this phase.",
    blockedReason: "Payment and reservation actions are forbidden.",
    safeNextStep: "Keep HOLD.",
    evidenceRef: "PHASE_75_TO_90_DRAFT_OUTBOX_SAFETY_EVIDENCE.md",
    externalWrite: false,
    sent: false,
    remoteCreated: false,
...
const DEFAULT_DRAFT_OUTBOX_SUMMARY = {
  total: DEFAULT_DRAFT_OUTBOX.length,
  waitingHuman: DEFAULT_DRAFT_OUTBOX.filter((item) => item.draftState === "waiting_human").length,
  held: DEFAULT_DRAFT_OUTBOX.filter((item) => item.draftState === "held").length,
  highOrCritical: DEFAULT_DRAFT_OUTBOX.filter(
    (item) => (["high", "critical"] as readonly string[]).includes(item.riskLevel),
  ).length,
  displayOnly: true,
  externalWrite: false,
  sent: false,
...
export const MOBILE_CONSOLE_DEFAULT_SNAPSHOT: MobileConsoleSnapshot = {
  ...
  draftOutbox: DEFAULT_DRAFT_OUTBOX,
  draftOutboxSummary: DEFAULT_DRAFT_OUTBOX_SUMMARY,
  stopHistory: [
    {
  ...
  nextHumanAction: "review Draft Outbox safety evidence before push GO",
  phaseProgress: "75% reached; 75 to 90 draft outbox safety layer in progress",
