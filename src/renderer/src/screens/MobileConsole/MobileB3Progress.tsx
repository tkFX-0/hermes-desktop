...
const tag = (color: string): React.CSSProperties => ({
  display: "inline-block", background: `${color}18`, border: `1px solid ${color}55`,
  borderRadius: 4, padding: "1px 7px", color, fontSize: 11, fontWeight: 600,
});
...
export default function MobileB3Progress({ snapshot }: Props): React.JSX.Element {
  ...
  const RESULT_COLORS: Record<string, string> = {
    CLEAN_B3_PASS: "#3fb950", PASS_WITH_TIMING_CAVEAT: "#d29922", STOP: "#f85149", pending: "#8b949e",
  };
  ...
  return (
    <div>
      <div style={card}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 16, fontWeight: 700, color: "#3fb950" }}>{b3Progress.current} / {b3Progress.required}</span>
          </div>
        ...
        <div style={{ background: "#0d1117", border: "1px solid #a371f7", borderRadius: 6, padding: "10px 12px", marginBottom: 12 }}>
          ...
          <div style={{ ...muted, marginTop: 4 }}>確認方法: iPhone Private Console real status observation</div>
          <div style={{ ...muted, marginTop: 2 }}>タイミングルール: {b3Progress.timingRule}</div>
        </div>
        {b3Progress.rustDeskDeprecated && (
          <div style={{ background: "rgba(248,81,73,0.07)", border: "1px solid rgba(248,81,73,0.3)", borderRadius: 6, padding: "8px 12px", marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: "#f85149" }}>旧Session-009 (PC/RustDesk Settings観察): 保留</div>
            <div style={{ fontSize: 11, color: "#8b949e", marginTop: 2 }}>iPhone Private Console Phase 2C 完了後に再設定</div>
          </div>
        )}
      ...
      <div style={{ ...card, background: "#0d1117" }}>
        <div style={{ fontSize: 12, color: "#8b949e" }}>Level 3: {snapshot.level3} — B3 5/5 完了後に人間GOが必要</div>
      </div>
    </div>
  );
}
